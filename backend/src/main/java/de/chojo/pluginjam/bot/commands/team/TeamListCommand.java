...
public class TeamListCommand {
    ...
    private Container renderPage(int page, List<Team> entries, DiscordLocale locale) {
        ...
        List<ContainerChildComponent> teamContents = entries.subList(start, end).stream().map(team -> {
            var membersContent = team.members().stream().map(teamMember -> String.format("- %s", MentionUtil.user(teamMember.userId()))).collect(Collectors.joining(" \n"));
            ...
            var contentString = """
                    ...
                    *%s*
                    ### %s:
                    *%s*
                    """.formatted(team.meta().getTeamName(), MentionUtil.role(team.meta().getRoleId()), messageResolver.resolve("word-description", locale), DefaultValues.getTeamProjectDescription(team, messageResolver, locale), messageResolver.resolve("word-project-url", locale), DefaultValues.getTeamProjectUrl(team, messageResolver, locale), messageResolver.resolve("word-members", locale), membersContent);
        ...
        teamContents.add(ActionRow.of(hasPrev ? Component.enabled("onPrev") : Component.disabled("onPrev"), net.dv8tion.jda.api.components.buttons.Button.of(ButtonStyle.SECONDARY, UUID.randomUUID().toString(), "%d / %d".formatted(currentPage + 1, maxPages), null), hasNext ? Component.enabled("onNext") : Component.disabled("onNext")));
