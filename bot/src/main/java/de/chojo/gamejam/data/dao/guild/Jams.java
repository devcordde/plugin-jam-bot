...
import de.chojo.gamejam.data.wrapper.jamEntity.JamCreator;
...
public class Jams {
    ...
    public void create(JamCreator jamCreator) {
        try (var conf = QueryConfiguration.getDefault().withSingleTransaction()) {
            conf.query("INSERT INTO jamEntity(guild_id) VALUES(?) RETURNING id")
                    ...
                    .map(id -> {
                        ...
                        conf.query("""
                                ...
                                .single(call().bind(id)
                                        ...
                                        .bind(times.jamEntity().startTimestamp())
                                        .bind(times.jamEntity().endTimestamp())
    ...
    public Optional<Jam> activeJam() {
        return query("""
                SELECT
                    id
                FROM jam_state s
                LEFT JOIN jamEntity j ON j.id = s.jam_id
                WHERE s.active
                    AND guild_id = ?
                LIMIT 1;
                """)
    ...
    public Optional<Jam> byId(int id) {
        return query("""
                SELECT id FROM jamEntity WHERE guild_id = ? AND id = ?
                """)
